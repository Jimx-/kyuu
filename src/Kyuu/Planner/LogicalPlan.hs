{-# LANGUAGE TemplateHaskell, FlexibleContexts, GADTs, RecordWildCards, NamedFieldPuns #-}
...
data LogicalPlan = DataSource { tableId :: OID
                              , tableName :: String
                              , searchArgs :: [SqlExpr Value]
                              , tupleDesc :: TupleDesc }
                 | Selection { conditions :: [SqlExpr Value]
                             , tupleDesc :: TupleDesc
...
buildProjection child exprs = Projection exprs tupleDesc child
    where
        tupleDesc =
                flip fmap exprs
                        $ \expr -> ColumnDesc (getOutputTableId expr)
